...
//	        });
	    ...
	    //shows dropdown list
	    showList: function() {
			//add lazyGetData process
			if (!this.config.has_lazy_data)
				this.lazyGetData();
	...
	lazyGetData:function(config){
		var data, i, max_len, html='';
		var self=this;
		...
		var on_success = function(data){
			...
			self.selectbox.html(html);
			self.options = self.selectbox.children().filter("option");
			self.addData();
			self.has_lazy_data = true;
		}
		
		if (!this.has_lazy_data && (this.config.lazyGetDataFn || this.config.lazyGetDataUrl)){
			if (this.config.lazyGetDataFn){
				this.config.lazyGetDataFn(on_success);
			}else{
				$.ajax({
				      type: 'POST',
				      url: this.config.lazyGetDataUrl,
				      dataType: 'json',
